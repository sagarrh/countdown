...
export const CountdownProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const defaultDate = new Date(`${new Date().getFullYear() + 1}-01-01T00:00:00.000Z`);
  ...
  const parseDate = (value: string | string[] | undefined): Date => {
    if (typeof value === "string") {
      const [year, month, day, hour = 0, minute = 0] = parseDateArguments(value);
  ...
  const [background, setBackground] = useState<BackgroundOption>(
    BackgroundOptions.find(
      (o) => o.value === parseString(searchParams.get("bg") || defaultBackground.value)
    ) || defaultBackground
  );
